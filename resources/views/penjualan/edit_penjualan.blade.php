          ...
          <li class="nav-item menu-items">
            ...
            <div class="collapse" id="ui-basic">
              <ul class="nav flex-column sub-menu">
                ...
                <li class="nav-item"> <a class="nav-link" href="/penjualan"> Transaksi Pembelian </a></li>
              ...
              <div class="col-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    ...
                    <div class="card">
                              <div class="card-body">
                                <form class="forms-sample" method="post" action="{{ url('/update_penjualan/'.$penjualan->id_transaksi) }}">
                                @csrf
                                @method('PUT')
                                {{csrf_field()}}
                                  <div class="form-group">
                                    <input type="hidden" name="kode" value="{{$penjualan->id_transaksi}}" class="fcontrol" id="id_transaksi" placeholder="Kode">
                                  <div class="form-group">
                                        <label for="id_transaksi">Karyawan</label>
                                        <select name="karyawan" class="form-select" id="j_penjualan">
                                        <option hidden value="{{$penjualan->id_karyawan}}" >{{$penjualan->karyawan->nama_karyawan}}</option>
                                  ...
                                  <div class="form-group">
                                        <label for="id_transaksi">Barang</label>
                                        <select name="barang" class="form-select" id="j_penjualan">
                                        <option hidden value="{{$penjualan->kode_barang}}" >{{$penjualan->barang->nm_barang}} = {{$penjualan->barang->harga}}</option>
                                        @foreach ($barang as $barang)
                                        <option value="{{$penjualan->kode_barang}}" >{{$barang->nm_barang}} = {{$barang->harga}}</option>
                                        @endforeach
                                        </select>
                                    </div>
                                  <div class="form-group">
                                        <label for="id_transaksi">Customer</label>
                                        <select name="customer" class="form-select" id="j_penjualan">
                                        <option hidden value="{{$penjualan->kode_customer}}" >{{$penjualan->customer->nama_customer}}</option>
                                        @foreach ($customer as $customer)
                                        <option value="{{$penjualan->kode_customer}}" >{{$customer->nama_customer}}</option>
                                        @endforeach
                                        </select>
                                    </div>
                                  <div class="form-group">
                                        <label for="id_transaksi">Outlet</label>
                                        <select name="outlet" class="form-select" id="j_penjualan">
                                        <option hidden value="{{$penjualan->kode_outlet}}" >{{$penjualan->outlet->nama_outlet}}</option>
                                        @foreach ($outlet as $outlet)
                                        <option value="{{$penjualan->kode_outlet}}" >{{$outlet->nama_outlet}}</option>
                                        @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                      <label for="nm_penjualan">Quantity</label>
                                      <input type="number" name="quantity" value="{{$penjualan->quantity}}" class="form-control" id="harga" placeholder="Harga">
                                    </div>
                                    <div class="form-group">
                                      <label for="nm_penjualan">Total</label>
                                      <input type="number" name="total" value="{{$penjualan->total_penjualan}}" class="form-control" id="harga" placeholder="Harga">
                                    </div>
                                    <div class="form-group">
                                      <label for="nm_penjualan">Date</label>
                                      <input type="date" name="date" value="{{$penjualan->tgl_penjualan}}" class="form-control" id="harga" placeholder="Harga">
                                    </div>
