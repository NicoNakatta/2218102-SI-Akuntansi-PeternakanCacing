              ...
              <div class="col-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    ...
                    <div class="card">
                              <div class="card-body">
                                <form class="forms-sample" method="post" action="{{ url('/update_pembelian/'.$pembelian->kode_po) }}">
                                @csrf
                                @method('PUT')
                                {{csrf_field()}}
                                  <div class="form-group">
                                    <input type="hidden" name="kode" value="{{$pembelian->kode_po}}" class="fcontrol" id="kode_po" placeholder="Kode">
                                  <div class="form-group">
                                        <label for="kode_po">Karyawan</label>
                                        <select name="karyawan" class="form-select" id="j_pembelian">
                                        <option hidden value="{{$pembelian->id_karyawan}}" >{{$pembelian->karyawan->nama_karyawan}}</option>
                                        @foreach ($karyawan as $karyawan)
                                        <option value="{{$pembelian->id_karyawan}}" >{{$karyawan->nama_karyawan}}</option>
                                        @endforeach
                                        </select>
                                    </div>
                                  <div class="form-group">
                                        <label for="kode_po">Barang</label>
                                        <select name="barang" class="form-select" id="j_pembelian">
                                        <option hidden value="{{$pembelian->kode_barang}}" >{{$pembelian->barang->nm_barang}} = {{$pembelian->barang->harga}}</option>
                                        @foreach ($barang as $barang)
                                        <option value="{{$pembelian->kode_barang}}" >{{$barang->nm_barang}} = {{$barang->harga}}</option>
                                        @endforeach
                                        </select>
                                    </div>
                                  <div class="form-group">
                                        <label for="kode_po">Supplier</label>
                                        <select name="supplier" class="form-select" id="j_pembelian">
                                        <option hidden value="{{$pembelian->kode_supplier}}" >{{$pembelian->supplier->nama_supplier}}</option>
                                        @foreach ($supplier as $supplier)
                                        <option value="{{$pembelian->kode_supplier}}" >{{$supplier->nama_supplier}}</option>
                                        @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                      <label for="nm_pembelian">Quantity</label>
                                      <input type="number" name="quantity" value="{{$pembelian->quantity}}" class="form-control" id="harga" placeholder="Harga">
                                    </div>
                                    <div class="form-group">
                                      <label for="nm_pembelian">Total</label>
                                      <input type="number" name="total" value="{{$pembelian->total_harga}}" class="form-control" id="harga" placeholder="Harga">
                                    </div>
                                    <div class="form-group">
                                      <label for="nm_pembelian">Date</label>
                                      <input type="date" name="date" value="{{$pembelian->tgl_pembelian}}" class="form-control" id="harga" placeholder="Harga">
                                    </div>
